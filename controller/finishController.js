...
exports.viewAll = async (req, res) => {
  try {
    ...
    const items = await Finish.find({}, fields).skip(skip).limit(limit).lean();
...
exports.viewById = async (req, res) => {
  try {
    const { id } = req.params;
    const item = await Finish.findById(id);
    if (!item) {
...
exports.update = async (req, res) => {
  ...
  try {
    const { id } = req.params;
    const { name } = req.body;
    const updated = await Finish.findByIdAndUpdate(id, { name }, { new: true });
    if (!updated) {
      return res.status(404).json({ success: false, message: "Not found" });
    }
